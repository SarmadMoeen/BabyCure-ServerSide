...
const RequestMotherCustomizeDietPlan = require('../models/reqMotherCustomizeDietPlan ')



//Here we will handle post request for add Milestones
router.post("/postMotherReqCustomizeDietPlan", async (req,res)=>{
    try{
        const addingReqCustomizeDietPlan = new RequestMotherCustomizeDietPlan()
    ...
    router.get("/getMotherCustomizeDietPlanReq", async (req,res)=>{
        try{
                const getCustomizeReq = await RequestMotherCustomizeDietPlan.find({})
    ...
    try{
            ...
            const getMilestone= await RequestMotherCustomizeDietPlan.find({_id})
...
//Here we handle the patch request for Milestone
    ...
    try{
                const _id = req.params.id
                const updateCustmizeReq = await RequestMotherCustomizeDietPlan.findByIdAndUpdate({_id} ,req.body,{new:true})
    ...
    try{
            const deleteCustmizeReq = await RequestMotherCustomizeDietPlan.findByIdAndDelete(req.params.id)
